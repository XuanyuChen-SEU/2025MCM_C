...
df_CHN = df[(df['Year'] >= 2000) & (df['NOCC'] == 'CHN')]

# 3. 数据分析
# 计算参与次数
participation = df_CHN.groupby('Sport')['Name'].count().reset_index()
...
def medal_to_score(medal):
    if medal == "Gold":
        ...
        return 0
df_CHN['Medal_Score'] = df_CHN['Medal'].apply(medal_to_score)

medal_count = df_CHN.groupby('Sport')['Medal_Score'].sum().reset_index()
...
# # 添加标题和标签
# plt.title('Participation and Medal Count of CHN in Each Sport Since 2000')
# plt.xlabel('Sport')
# plt.ylabel('Count')
...
# 添加标题和标签
plt.title('Medal Ratio of CHN in Each Sport Since 2000')
