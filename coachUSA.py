...
df_usa = df[(df['Year'] >= 2000) & (df['NOCC'] == 'USA')]

# 3. 数据分析
# 计算参与次数
participation = df_usa.groupby('Sport')['Name'].count().reset_index()
...
def medal_to_score(medal):
    if medal == "Gold":
        ...
        return 0
df_usa['Medal_Score'] = df_usa['Medal'].apply(medal_to_score)

medal_count = df_usa.groupby('Sport')['Medal_Score'].sum().reset_index()
...
# # 添加标题和标签
# plt.title('Participation and Medal Count of USA in Each Sport Since 2000')
# plt.xlabel('Sport')
# plt.ylabel('Count')
...
# 添加标题和标签
plt.title('Medal Ratio of USA in Each Sport Since 2000')
